// client/src/pages/Dashboard.jsx - WITH PDF REPORT GENERATION
  ...
  const generatePDFReport = async () => {
    ...
    try {
      ...
      const avgScore = totalJobs > 0 
        ? Math.round(jobs.reduce((sum, j) => sum + (j.analysis?.overallScore || 0), 0) / totalJobs)
        : 0;
      
      const smartBuilderJobs = jobs.filter(j => j.type === 'smart-builder').length;
      const manualEntryJobs = jobs.filter(j => j.type === 'manual-entry').length;
      const excellentJobs = jobs.filter(j => (j.analysis?.overallScore || 0) >= 80).length;
      const goodJobs = jobs.filter(j => {
        const score = j.analysis?.overallScore || 0;
        return score >= 60 && score < 80;
      }).length;
      const needsWorkJobs = jobs.filter(j => (j.analysis?.overallScore || 0) < 60).length;
      ...
      const response = await axios.post('http://localhost:5000/api/reports/generate-pdf', {
        ...
        userData: {
          ...
          email: userEmail,
          company: localStorage.getItem('userCompany') || 'Company',
        ...
        jobs: jobs.map(job => ({
          ...
          type: job.type,
          status: job.status,
          score: job.analysis?.overallScore || 0,
          createdAt: job.createdAt,
          location: job.location,
          workCondition: job.workCondition,
      ...
      link.href = url;
      link.download = `Guhuza_Report_${new Date().toISOString().split('T')[0]}.pdf`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 flex flex-col transition-colors duration-200">
      ...
      <div className="flex-grow max-w-7xl mx-auto px-4 py-8 w-full">
        ...
        <div className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl shadow-lg p-6 mb-6 border-2 border-purple-200 dark:border-purple-800">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100">Activity Report</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400">Download comprehensive PDF report of all your job postings</p>
              </div>
            </div>
            
            <button
              onClick={generatePDFReport}
              disabled={isGeneratingPDF || isLoading || jobs.length === 0}
              className="bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold px-8 py-3 rounded-xl hover:from-purple-700 hover:to-pink-700 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-lg transform hover:scale-105 flex items-center whitespace-nowrap"
            >
              {isGeneratingPDF ? (
                <>
          ...
          {jobs.length === 0 && (
            <p className="text-center text-sm text-gray-500 dark:text-gray-400 mt-3">
              Create some job postings first to generate a report
            </p>
          )}
        </div>
